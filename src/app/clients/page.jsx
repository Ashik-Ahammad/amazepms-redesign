...
import { Building2, Home, ShoppingBag, HeartPulse, PackageSearch, GraduationCap, Factory, Globe2, ArrowUpRight } from "lucide-react";
    ...
    return (<main className="pt-20 bg-background text-foreground selection:bg-brand/30">
      ...
      <section className="relative py-24 lg:py-32 overflow-hidden bg-surface">
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8 relative z-10 text-center">
          <motion.div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-brand/30 bg-brand/10 mb-8" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
          ...
          <motion.h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-tight text-foreground mb-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
            Empowering <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand to-emerald-500">Industry Leaders</span>
          </motion.h1>
          
          <motion.p className="text-muted text-lg md:text-xl leading-relaxed max-w-3xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2, duration: 0.6 }}>
      ...
      <section className="py-24 bg-background border-y border-border relative overflow-hidden">
        <motion.div className="mx-auto max-w-7xl px-6 lg:px-8" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.7, ease: "easeOut" }}>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center max-w-5xl mx-auto">
            ...
            <div className="h-[400px] w-full relative">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={PIE_DATA} cx="50%" cy="50%" innerRadius={90} outerRadius={140} paddingAngle={5} dataKey="value" stroke="none">
                    {PIE_DATA.map((entry, index) => (<Cell key={`cell-${index}`} fill={entry.color}/>))}
                  </Pie>
                  <Tooltip formatter={(value) => [`${value}%`, "Market Share"]} contentStyle={{ backgroundColor: 'hsl(var(--surface))', borderColor: 'hsl(var(--border))', borderRadius: '8px', color: 'hsl(var(--foreground))' }} itemStyle={{ fontWeight: 'bold' }}/>
                </PieChart>
              </ResponsiveContainer>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-3">
                {PIE_DATA.map((item, index) => (<div key={index} className="flex items-center justify-between p-4 rounded-xl border border-border bg-surface hover:border-brand/30 transition-colors">
      ...
      <section className="py-32 bg-surface-elevated relative">
        <motion.div className="mx-auto max-w-7xl px-6 lg:px-8" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.7, ease: "easeOut" }}>
          ...
          <div className="flex flex-wrap justify-center gap-3 mb-16">
            {CLIENT_CATEGORIES.map((category) => (<button key={category.id} onClick={() => setActiveCategory(category.id)} className={cn("flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold transition-all duration-300", activeCategory === category.id
          ...
          <div className="min-h-[400px]">
            {CLIENT_CATEGORIES.map((category) => (activeCategory === category.id && (<motion.div key={category.id} initial={{ opacity: 0, scale: 0.95, filter: "blur(10px)" }} animate={{ opacity: 1, scale: 1, filter: "blur(0px)" }} exit={{ opacity: 0, scale: 0.95, filter: "blur(10px)" }} transition={{ duration: 0.4, ease: "easeOut" }} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {category.clients.map((client, idx) => (<motion.div key={idx} className="group relative flex items-center justify-between p-4 rounded-2xl border border-border bg-background hover:bg-surface hover:border-brand/50 transition-all duration-300 cursor-default overflow-hidden" whileHover={{ y: -2 }}>
      ...
      <section className="py-20 bg-background relative overflow-hidden border-t border-border">
        <motion.div className="mx-auto max-w-7xl px-6 lg:px-8 text-center mb-10" initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
        ...
        <motion.div className="relative [mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)]" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 1, delay: 0.2 }}>
          <Marquee gradient={false} speed={40} className="py-4">
            {[...CLIENT_CATEGORIES[0].clients, ...CLIENT_CATEGORIES[1].clients, ...CLIENT_CATEGORIES[2].clients].map((client, idx) => (<div key={idx} className="flex items-center gap-8 mx-8 group cursor-default">
