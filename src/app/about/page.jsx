...
import { Shield, Target, Heart, CheckCircle2, Award, Cpu, UserCheck, ShieldAlert, Sparkles } from "lucide-react";
    ...
    return (<main className="pt-20">
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <motion.div 
              ...
              className="p-8 rounded-3xl glass border border-border hover:border-brand/30 hover:-translate-y-2 transition-all duration-300 group">
      ...
      <section className="py-24 bg-surface-elevated relative overflow-hidden">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <SectionHeading badge="Why Choose Amaze" title="What Sets Us" titleHighlight="Apart" align="center" className="mb-16"/>

          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {DIFFERENTIATORS.map((diff, i) => (<motion.div key={i} 
                initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.5, delay: i * 0.1 }}
                className="p-6 rounded-2xl border border-border bg-surface hover:bg-surface-glass hover:border-brand/40 transition-all duration-300">
      ...
      <section className="py-24 bg-background relative overflow-hidden">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <SectionHeading badge="Our Journey" title="Two Decades of" titleHighlight="Growth" align="center" className="mb-16"/>

          <div className="grid md:grid-cols-4 gap-6">
            {TIMELINE.map((item, index) => (<motion.div key={index} 
                initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ duration: 0.6, delay: index * 0.15 }}
                className="relative p-6 rounded-2xl border border-border bg-surface-glass flex flex-col justify-between">
      ...
      <section className="py-24 relative bg-surface-elevated overflow-hidden">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {SEGMENTS.map((segment, idx) => (<motion.div key={segment} 
                initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-20px" }} transition={{ duration: 0.4, delay: idx * 0.05 }}
                className="flex items-center gap-3 p-4 rounded-xl border border-border bg-surface hover:bg-surface-glass hover:border-brand/30 transition-all duration-300 group cursor-default">
