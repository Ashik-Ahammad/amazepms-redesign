      ...
      <section className="py-24 bg-background">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="grid md:grid-cols-2 gap-x-12 gap-y-6">
            {STRENGTHS.map((strength, index) => (<motion.div key={index} className="flex items-start gap-4 p-4 rounded-xl hover:bg-surface-elevated transition-colors duration-300" initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.05 }}>
      ...
      <section className="py-24 bg-surface-elevated relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-6 lg:px-8 relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {WELFARE_BENEFITS.map((benefit, index) => (<motion.div key={index} className="p-6 rounded-2xl glass border border-border group" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.1 }}>
            ...
            {/* Highlight Card */}
            <motion.div className="p-6 rounded-2xl bg-gradient-to-br from-brand-dark to-brand flex flex-col justify-center sm:col-span-2 lg:col-span-1" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.5 }}>
