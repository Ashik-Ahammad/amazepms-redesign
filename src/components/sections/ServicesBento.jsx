...
export function ServicesBento() {
    return (<section id="services" className="relative py-24 sm:py-32 overflow-hidden">
      {/* Ambient light */}
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-8">
        <SectionHeading badge="Our Services" title="Integrated Property" titleHighlight="Management Solutions" description="From security to horticulture, we deliver end-to-end facility management services tailored to your property's unique needs."/>

        {/* Bento Grid */}
        <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-5" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-100px" }}>
          {ALL_SERVICES_EXTENDED.map((service, index) => {
            ...
            return (<motion.div key={service.title} className={`group relative rounded-2xl border border-border bg-surface-glass p-6 sm:p-8 overflow-hidden transition-all duration-500 hover:border-brand/20 hover:shadow-xl hover:shadow-brand/5 ${span}`} variants={cardVariants}>
                ...
                <div className="relative z-10">
                  <div className="w-12 h-12 rounded-xl bg-brand/10 flex items-center justify-center mb-5 group-hover:bg-brand/20 transition-colors duration-300">
                    {Icon && (<Icon className="w-6 h-6 text-brand-light group-hover:text-brand transition-colors duration-300"/>)}
                  </div>
