      ...
      <div className="mx-auto max-w-[1400px] px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
          ...
          <div className="flex gap-3">
            <button onClick={scrollPrev} disabled={!prevBtnEnabled} className="w-12 h-12 rounded-full border border-border bg-surface-glass hover:bg-surface-elevated hover:border-brand/40 flex items-center justify-center text-foreground transition-all disabled:opacity-50 disabled:cursor-not-allowed group">
              <ChevronLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform"/>
            </button>
            <button onClick={scrollNext} disabled={!nextBtnEnabled} className="w-12 h-12 rounded-full border border-border bg-surface-glass hover:bg-surface-elevated hover:border-brand/40 flex items-center justify-center text-foreground transition-all disabled:opacity-50 disabled:cursor-not-allowed group">
        ...
        <div className="overflow-hidden -mx-4 px-4 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8 py-4" ref={emblaRef}>
          <div className="flex touch-pan-y touch-pinch-zoom gap-6">
            ...
            const Icon = LucideIcons[service.icon] || LucideIcons.Building2;
            return (<motion.div key={service.title} className="relative flex-[0_0_85%] sm:flex-[0_0_45%] lg:flex-[0_0_30%] xl:flex-[0_0_25%] h-[400px] rounded-[2rem] overflow-hidden group cursor-grab active:cursor-grabbing border border-border/50 shadow-lg" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.6, delay: index * 0.1 }}>
                  {/* Background Image with Slow Zoom on Hover */}
                  <Image src={service.image} alt={service.title} fill className="object-cover transform transition-transform duration-[2000ms] ease-out group-hover:scale-110 saturate-[1.1] contrast-[1.1] brightness-90" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"/>
