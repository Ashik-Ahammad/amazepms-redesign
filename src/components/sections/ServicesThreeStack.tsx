...
function StackImage({ url, index, total }: { url: string; index: number; total: number }) {
  ...
  useFrame((state, delta) => {
    ...
    if (distance > 0) {
      ...
      (material as any).opacity = damp((material as any).opacity, targetOpacity, 4, delta);
      ...
      (material as any).opacity = damp((material as any).opacity, targetOpacity, 6, delta);
    } else {
      // Hidden completely
...
function Scene({ services }: { services: typeof ALL_SERVICES_EXTENDED }) {
  ...
  return (
    <group position={[stackPositionX, 0, 0]}>
...
export function ServicesThreeStack() {
  ...
  return (
    <div className="w-full h-[100dvh] bg-background relative overflow-hidden" data-lenis-prevent="true">
          ...
          <ScrollControls pages={stackServices.length} damping={0.15} distance={1.2}>
            ...
            <Scroll html style={{ width: "100%", height: "100%" }}>
              {stackServices.map((service, index) => (
                <div 
                  key={service.title} 
                  className="w-full h-screen flex items-center justify-start pointer-events-none px-6 md:px-24"
                >
                  {/* Text Overlay */}
                  <div className="max-w-xl flex flex-col items-start pointer-events-auto pb-40 md:pb-0">
                    <div className="inline-flex items-center gap-2 mb-4 px-3 py-1.5 rounded-full border border-border bg-surface-glass backdrop-blur-md shadow-sm">
                      {(() => {
                        const Icon = (LucideIcons as any)[service.icon] || LucideIcons.ArrowRight;
