...
import {
  BarChart3, Users, ClipboardCheck, ShieldCheck,
  HeartHandshake, Cog, TrendingUp, Building,
} from "lucide-react";
...
export function Features() {
  return (
    <section id="why-us" className="relative py-24 sm:py-32 bg-surface overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <motion.div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-80px" }}>
          {features.map((feature) => (
            <motion.div key={feature.title} className="group relative p-6 rounded-2xl border border-border bg-background hover:bg-surface-glass hover:border-brand/15 transition-all duration-500" variants={itemVariants}>
